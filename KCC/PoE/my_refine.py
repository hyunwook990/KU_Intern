...
작업:
1. 문제의 핵심 조건과 요구사항을 먼저 파악하세요.
2. 각 선택지가 문제의 조건에 얼마나 부합하는지 내부적으로 단계별 비교하세요.
3. 오답 가능성이 큰 선택지를 배제하고, 남은 선택지 중 가장 타당한 최종 정답 하나를 고르세요.
4. 내부 추론 과정은 출력하지 말고, 최종 판단 근거만 2~4문장 explanation으로 요약하세요.

반드시 아래 JSON 형식으로만 답하세요:
{{
  "answer": "라벨",
  "explanation": "최종 판단 근거 요약",
  "confidence": 0.0
}}
...
- explanation에는 최종 선택의 핵심 근거만 쓰세요.
- 단계별 사고 과정이나 장황한 추론은 출력하지 마세요.
- JSON 이외의 텍스트는 출력하지 마세요.
"""
...
class EliminationPipeline:
    ...
    def _restore_top2_with_ties_if_single_remaining(
        ...
        sorted_unique_scores = sorted(
            set(rounded_confidences),
            reverse=True
        )
            ...
            trace.append({
                ...
                "first_elimination": {
                    ...
                    "original_remaining_labels": self._labels_from_global_indices(
                        self._subset_by_indices(current_global_indices, first_result.remaining_indices)
                    ),
                    "final_decision_candidate_labels": remaining_labels,
                    "final_decision_candidate_options": remaining_options,
                    "final_decision_candidate_rationales": remaining_rationales,
                    "final_decision_candidate_confidences": remaining_confidences,
                    "eliminated_records": [
                        {
                            "label": r.label,
