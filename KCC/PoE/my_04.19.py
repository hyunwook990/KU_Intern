...
2. rationale은 반드시 한 문장으로만 작성하세요.
...
def format_reason_block(
    ...
    for i in range(len(labels)):
        if confidences is None:
            ...
            lines.append(
                f"- {labels[i]}. {options[i]}: {rationales[i]} (conf={confidences[i]:.2f})"
            )
    return "\n".join(lines)
...
class FinalDecision:
    ...
    def run(
        ...
        try:
            ...
            final_explanation = remaining_rationales[chosen_idx]
    ...
    for idx, sample in enumerate(dataset):
        try:
            ...
            if verbose:
                ...
                print("final_confidence:", output["final"]["confidence"])
...
if __name__ == "__main__":
    ...
    # save_results_json(result, "my_0419.json")